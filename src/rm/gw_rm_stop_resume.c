...
void gw_rm_stop_failed (void *_job_id)
{
	...
	connection = gw_connection_list_get(&(gw_rm.connection_list),
										...
										job_id);
	if ( connection == NULL )
		gw_log_print("RM",'W',"Connection for job %i has been closed (STOP_FAILED).\n",
		             job_id);	
	else
		while ( connection != NULL )
		{			
...
void gw_rm_resume_success(void *_job_id)
{
	...
	connection = gw_connection_list_get(&(gw_rm.connection_list),
										...
										job_id);
	if ( connection == NULL )
		gw_log_print("RM",'W',"Connection for job %i has been closed (RESUME_SUCCESS).\n",
		             job_id);	
	else
		while ( connection != NULL )
		{			
